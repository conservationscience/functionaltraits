...
db_wilman_bird_eltontraits <- setRefClass( 
  "db_wilman_bird_eltontraits",
  contains = "Database",
  
  methods = list(
    
    filename = function() {
      return( file.path( dir, "BirdFuncDat.txt") )
    },
    ...
    ready = function() {
      if( file.exists( .self$filename() ) )
        ...
        warning( "the database file for wilman_bird_eltontraits has not been downloaded")
        return( FALSE )
      }
    },
